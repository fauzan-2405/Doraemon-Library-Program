# FILE INI DIPAKE BUAT NGE READ FILE RIWAYAT PENGEMBALIAN GADGET
# JADINYA NANTI KITA NGE IMPORT data_pengembalian_gadget DARI FILE INI
# KLO DIBUAT SATU SATU RIWEH EUY
from f14 import Load
f = open(f"{Load.load}/gadget_return_history.csv","r")
...
# Fungsi ini digunakan untuk mengganti nilai data ke nilai real(asli) nya
    ...
    for i in range (3):
        if (i==0) or (i==1):
            # Kolom dgn index (0) dan 1 adalah "id" dan "id_peminjaman"
...
data_pengembalian_gadget1 =[]
header_data_pengembalian_gadget = lines.pop(0)
header_data_pengembalian_gadget = ganti_line_ke_data(header_data_pengembalian_gadget)

for line in lines:
    array_of_data = ganti_line_ke_data(line)
    data_real = ganti_data_ke_real(array_of_data)
    data_pengembalian_gadget1.append(data_real)

data_pengembalian_gadget = data_pengembalian_gadget1